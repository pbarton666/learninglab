...
Serialize and save them at 1-second intervals using time.sleep().
Produce a report using os.stat() that verifies the time stamps were about a
second apart.
...
#Now, check the timestamps to ensure they're close to 1sec apart

times=[]	
for name in the_pickle_names:
	stat_result=os.stat(name).st_ctime
	times.append(stat_result)

first_time=times[0]
total_time=0
for item, ctime in enumerate(times):
	if item > 0:  #
		delta=(ctime-first_time) / 1000 #time here is in ms
		total_time += delta
		fstg="elapsed time to object# {} creation: {}"
		print(fstg.format(round(item,2), round(delta, 2)))
		
avg= total_time/(len(times) - 1) 
fstg="The timeout was set to {} sec\n... and the average lapse was {} sec."
print(fstg.format(time_out, round(avg,2))) 
